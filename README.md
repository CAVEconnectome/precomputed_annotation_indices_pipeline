...
2. Create a data config json file. The name doesn't matter, so long as it ends with a '.json' extension.
	...
	2. Amongst other aspects of the data for this template, you will need:
		...
		2. The spatial point lower and upper bounds. This is easiest — if a bit tedious — by line-scanning the CSV file to find the data bounds. If this is assigned null or empty list, the pipeline will calculate it for you, but it can take a long time, so provide it if you can. Alternatively, if the pipeline must calculate the volume bounds, it will write those bounds to the logging output. At that time, you can copy and paste it into the data config file for subsequent runs, which will significantly expedite those runs relative to the initial run.
